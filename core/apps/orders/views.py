...
def send_notification(user, title, message, notif_type=Notification.IN_APP):
    """Helper: creates a Notification record for a user."""
...
def authorize_commission(order):
    """
    Creates a CommissionPayment and calls Paymob to authorize the commission.

    On success → saves CommissionPayment with status=AUTHORIZED
    On failure → saves CommissionPayment with status=FAILED (order is NOT blocked)

    Returns payment_key string (for the frontend iframe), or None if Paymob failed.
    """
    ...
    try:
        ...
        logger.info(f"Commission AUTHORIZED — Order #{order.id}, Paymob order: {paymob_order_id}")
        return payment_key
...
class OrderListCreateView(APIView):
    ...
    def get(self, request):
        ...
        if user.role == User.Role.CLIENT:
            ...
            orders = Order.objects.all()   # admin sees everything
    ...
    def post(self, request):
        ...
        for wp in available_workers:
            if order.worker and order.worker == wp.user:
                ...
                message = (
                    f"A client chose you for a {order.service_category.name} order #{order.id}. "
                    f"Price: {order.price} EGP. Please accept or reject."
                )
            else:
                title   = "New Order Available"
                message = (
                    f"New {order.service_category.name} order #{order.id}. "
                    f"Price: {order.price} EGP."
                )
...
class OrderAcceptView(APIView):
    ...
    def post(self, request, pk):
        ...
        # Capture commission via Paymob
        # Only possible after the webhook has delivered paymob_transaction_id
        try:
